        ...
        if (resultCode == Activity.RESULT_OK) {
            if (requestCode == REQUEST_PICK) {//第一次，选择图片后返回
                ...
                if (photos == null || photos.size()==0){
                    builder.callback.onFail("未选择图片",new Throwable("未选择图片"));
                    return;
                }
                    ...
                    String path = photos.get(0);
                     if(builder.needCropWhenOne){//需要裁剪(裁剪内部自带压缩功能),就跳到裁剪的页面
                        ...
                        MyTool.compressPics(builder);
                         ...
                         builder.callback.onSuccessSingle(path,path);
                     }
                }else {//选择了多图
                    if(!builder.needCompress){//不需要压缩
                        builder.callback.onSuccessMulti(photos,photos);
                        return;
                    }
                    //多图压缩
                    //todo 压缩
                ...
                if(finalUri == null || (!new File(builder.filePathToCrop).exists())){
                    builder.callback.onFail("图片裁剪失败1",new Throwable("图片裁剪失败1"));
                }else {
                    ...
                    builder.callback.onSuccessSingle("dddd",builder.filePathToCrop);
                }
                ...
                if(!new File(path).exists()){
                    builder.callback.onFail("拍照失败:"+builder.filePathToCamera,new Throwable("拍照失败:"+builder.filePathToCamera));
                    return;
                }
